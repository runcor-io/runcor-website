        ...
        try:
            with zipfile.ZipFile(zip_path, 'r') as zf:
                ...
                for info in infolist:
                    ...
                    # Security: Block absolute paths and path traversal attempts
                    if filename.startswith('/') or '..' in filename:
                        blocked_files.append(f"{filename} (absolute/traversal path)")
                        continue
                    
                    # NOTE: Symlink checking temporarily disabled - add back if symlink attacks observed
